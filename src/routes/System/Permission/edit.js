        ...
        this.props.dispatch({
          ...
          callback: (resp) => {
            if (resp.returnCode !== 200) {
              ...
              this.props.dispatch(routerRedux.push('/system/permission'));
            }
          },
        });
      }
    ...
    const loop = (data) => {
      return data.map((item) => {
        if (item.children && item.children.length > 0) {
          return <TreeNode key={item.id} value={`${item.id}`} title={item.fullName}><TreeNode key="0" value="0" title="根节点">{loop(item.children)}</TreeNode></TreeNode>;
        }
